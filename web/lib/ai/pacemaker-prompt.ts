...
사용자 본인의 자유 답변에 집중. suggestedAnswers는 항상 빈 배열.`;
  ...
  // Q2 차례(turn=1)일 때 — LLM이 history에 보이는 Q1+사용자 답변을 보고 후속
  // 질문을 만들려는 경향이 강하므로 system 끝부분에 다시 한 번 강조.
  if (args.turn === 1) {
    out += `
...
위에 보이는 history(Q1 + 사용자 답변)은 컨텍스트일 뿐, **Q2는 Q1과 완전히 분리된 새 질문**입니다.

🚫 **금지 (이 케이스가 가장 흔한 실패 패턴):**
- "그 [Q1 답변 키워드]을 ~하는 과정에서…" ← Q1 답변 인용으로 시작
- "그 [Q1 키워드]가 다른 순간에도?" ← Q1 결을 다른 시점으로
- Q1 답변 표현을 따옴표로 다시 가져와 질문에 박기

✅ **해야 할 것:**
- Q1과 다른 영역으로 시선 이동 (일↔관계↔취미↔휴식↔신앙)
- baseline의 다른 Part 키워드 환기
- 최근 답변에 아직 안 나온 결
- 사용자 답변에 등장한 어떤 단어도 Q2에 다시 쓰지 말 것

**한 줄 점검:** Q2 작성 직후 스스로 물어보세요 — "이 질문이 Q1 답변 모르더라도 자연스러운 첫 질문으로 던질 수 있는가?" Yes면 OK. No면 Q1 후속이니 다시.`;
  }
