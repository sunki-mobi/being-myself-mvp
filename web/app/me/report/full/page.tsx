...
/**
 * /me/report/full — 풀 셀프인터뷰 보고서 (3 Part 전체).
 *
 * 박람회 데모 컨텍스트에서는 매일 답변 한두 개만 보여주는 /me/report 가 기본.
 * 이 페이지는 "전체 보고서 보기" CTA로만 진입. 콘텐츠 양이 많으니 Part 탭으로
 * 한 번에 한 Part씩 보여주고 코치인터뷰는 토글로 숨김.
 */
      ...
      <div className="w-full max-w-md flex flex-col bg-surface-light text-fg-light lg:rounded-3xl lg:my-8 lg:min-h-[820px] lg:shadow-xl lg:shadow-brand-200/30">
        ...
        <nav className="px-6 pt-8 pb-2">
          <div className="flex gap-2 overflow-x-auto -mx-6 px-6 pb-1">
            {BASELINE_REPORT.parts.map((p, idx) => (
              <button
                key={idx}
                type="button"
        ...
        <section className="flex-1 px-6 py-6">
          <PartBody key={activePart} part={BASELINE_REPORT.parts[activePart]} />
        </section>

        {BASELINE_REPORT.closingNote ? (
